...
def test_proxies_batch(proxies, target_count, config):
    ...
    def test_and_check(proxy):
        ...
        if tested % 5 == 0 or tested == total:
            ...
            print(f"\r   进度：{tested}/{total} ({progress_pct:.1f}%) | 通过：{valid_count} | 失败：{tested-valid_count}", end='', flush=True)
...
def main():
    ...
    if not YF_AVAILABLE:
        print("❌ 错误：缺少依赖，请运行：pip3 install yfinance --break-system-packages")
        return 0
