...
if __name__ == "__main__":
    ...
    cash, final_value = run_backtest_in_process(
        trade_date, lambda d: BacktraderExec("us", d).exec_btstrategy(force_run=False)
    )
    ...
    cash, final_value = run_backtest_in_process(
        trade_date,
        lambda d: BacktraderExec("us_special", d).exec_btstrategy(force_run=False),
    )
    collected = gc.collect()
    ...
    cash, final_value = run_backtest_in_process(
        trade_date,
        lambda d: BacktraderExec("us_dynamic", d).exec_btstrategy(force_run=False),
    )
    collected = gc.collect()
