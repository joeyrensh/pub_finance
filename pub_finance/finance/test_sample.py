...
if __name__ == "__main__":
    ...
    cash, final_value = run_backtest_in_process(
        trade_date, lambda d: BacktraderExec("cn", d).exec_btstrategy(force_run=False)
    )
    ...
    cash, final_value = run_backtest_in_process(
        trade_date,
        lambda d: BacktraderExec("cnetf", d).exec_btstrategy(force_run=False),
    )
    collected = gc.collect()
    print("Garbage collector: collected %d objects." % (collected))
    """ 发送邮件 """
    ...
    cash, final_value = run_backtest_in_process(
        trade_date,
        lambda d: BacktraderExec("cn_dynamic", d).exec_btstrategy(force_run=False),
    )
    collected = gc.collect()
    print("Garbage collector: collected %d objects." % (collected))
    """ 发送邮件 """
