...
spark = SparkSession.builder \
      ...
      .appName("SparkTest") \
      .getOrCreate()
df = spark.read.csv('./usstockinfo/position_20221007.csv', header=True)
df.createOrReplaceTempView('temp')
sqlDF = spark.sql('select buy_date, industry, count(*) as cnt from temp group by buy_date, industry')
sqlDF.createOrReplaceTempView('temp_latest')
sqlDF1 = spark.sql("select * from temp_latest where buy_date = (select max(buy_date) from temp_latest) order by cnt desc limit 3")
sqlDF1.createOrReplaceTempView('temp_lastday')
sqlDF2 = spark.sql("select a.industry, coalesce(b.cnt, 0, b.cnt) as cnt, b.buy_date from temp_lastday a left outer join temp_latest b on a.industry = b.industry order by b.buy_date\
                ")
