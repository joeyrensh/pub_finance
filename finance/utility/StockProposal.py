...
class StockProposal:
    ...
    def send_strategy_df_by_email(self, dataframe):
        ...
        """ 匹配行业信息 """
        df_ind = pd.read_csv(file_name_industry,
        ...
        df_n.rename(columns={'symbol': '股票代码',
                             'close': '收盘价',
                             'chg': '涨幅',
                             'volume': '成交量',
                             'amplitude': '振幅',
                             'tag': '标记',
                             'industry': '所属行业'}, inplace=True)
    ...
    def send_btstrategy_by_email(self):
        ...
        df_pre_p = pd.read_csv(pre_file_cur_p, usecols=[
        ...
        if not df_cur_p.empty:
            ...
            html = (
                ...
                .bar(subset=['变化比'], align='left', color=['#5fba7d', '#d65f5f'], vmin=-1, vmax=1)
                .set_properties(**{'text-align': 'left', 'width': 'auto'})
                .set_table_styles([dict(selector='th', props=[('text-align', 'left')], width='auto')])
                .render()
            )
