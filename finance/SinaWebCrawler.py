...
class SinaWebCrawler:
    ...
    __url1 = "https://stock.finance.sina.com.cn/usstock/api/jsonp.php/IO.XSRV2.CallbackList" \
             "/US_CategoryService.getList?page=page_num&num=60&sort=&asc=0"
    # 分时股票url
    __url2 = "https://hq.sinajs.cn/?rn=unix_time&list=gb_list"

    # 5min趋势url
    __url3 = "https://stock.finance.sina.com.cn/usstock/api/jsonp_v2.php/var%20_ticker_5_unix_time=" \
             "/US_MinKService.getMinK?symbol=ticker&type=5"

    # 获取日交易最新数据
    @classmethod
    def get_daily_tick_info(cls, url):
        # 字典结构
        ...
        res_p = re.sub('\\\\', '', re.sub('/.*/', '', re.sub('}\\);$', '', re.sub('.*"data":', '', res, 1))))
    ...
    @classmethod
    def set_daily_tick_info_to_csv(cls, trade_date, wpath):
        ...
        for h in range(1, total_pages, 3):
            ...
            for t in range(1, 4):
                if (h+t) <= total_pages:
                    url1 = str(SinaWebCrawler.__url1).replace('page_num', str(h+t-1))
                    my_thread = MyThread(SinaWebCrawler.get_daily_tick_info, (url1,))
    ...
    def set_realtime_tick_into_to_csv(cls, rpath, wpath):
        ...
        for index, i in df.iterrows():
            # 取成交量大于0 且有股票代码作为循环列表，url参数里需要股票列表为list，且代码为小写
    ...
    @classmethod
    def get_5min_tick_info(cls, trade_date, url, ticker):
        # 字典结构
        # 股票代码：symbol，今日开盘价：open_alias，今日收盘价：close，今日最高价：high
        # 今日最低价：low，昨日收盘价：preclose，涨跌额：change，涨跌幅：chg
        # 今日成交量：volume，今日振幅：amplitude，
        ...
        for h in range(0, len(tickers), 3):
            ...
            for t in range(1, 4):
                if (h+t) <= len(tickers):
                    ...
                    url1 = str(SinaWebCrawler.__url3).replace('unix_time', str(current_timestamp)).\
                        replace('ticker', tickers[h+t-1])
                    # 启动多线程获取
                    my_thread = MyThread(SinaWebCrawler.get_5min_tick_info, (trade_date, url1, tickers[h+t-1]))
                    my_thread.setDaemon(True)
                    my_thread.start()
                    list1 = my_thread.get_result()
                    if list1 is None:
