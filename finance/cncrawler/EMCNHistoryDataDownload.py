...
class EMCNHistoryDataDownload:
    ...
    def get_his_tick_info(self, mkt_code, symbol, start_date, end_date):
        ...
        url_re = url.replace('mkt_code', mkt_code)\
    ...
    def set_his_tick_info_to_csv(self, start_date, end_date, file_path):
        ...
        for h in range(0, len(tickinfo), 4):
            ...
            for t in range(1, 5):
                if (h+t) <= len(tickinfo):
                    my_thread = MyThread(
                        self.get_his_tick_info, (tickinfo[h+t-1]['mkt_code'], tickinfo[h+t-1]['symbol'], start_date, end_date))
...
"""
    ...
    start_date = '20210101'
    end_date = '20211112'
    file_path = './cnstockinfo/stock_20211112.csv'
